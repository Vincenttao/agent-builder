...
- **新增**：运行阶段接入真实 LLM（API key 通过环境变量注入 Docker 沙箱）
- **新增**：生成 Agent 使用真实 OpenJiuwen 框架（ReActAgent + @tool + invoke），不再生成 mock/stub 适配层
- **新增**：`feature/openjiuwen-real` 分支（完整 openjiuwen 0.1.15）+ `demo/lightweight-runtime` 分支（自研轻量适配器）
...
| # | 项目 | 优先级 | 状态 | 说明 |
|---|---|---|---|---|
| **R1** | **运行阶段必须使用真实 LLM** | P0 | `feature/openjiuwen-real` 已实现，main 已实现 | 生成的 Agent/Workflow 在测试台运行时，必须通过真实 LLM API 驱动 ReAct 循环，不得返回 mock/占位文案。API key 通过环境变量注入，禁止硬编码。 |
| **R2** | **生成的 Agent 必须基于 OpenJiuwen 框架** | P0 | `feature/openjiuwen-real` 已实现 | 生成代码必须 import `openjiuwen.core.single_agent.ReActAgent`，使用 `@tool` 装饰器，调用 `agent.invoke()`。不得自己实现 Agent 循环、生成 mock adapter、或使用纯规则引擎。 |
| **R3** | **Agent 必须包含真正的 ReAct 循环** | P0 | `feature/openjiuwen-real` 已实现 | 生成 Agent 的 `run_agent()` 必须通过 LLM 的多轮 tool calls 驱动：Think（LLM 推理）→ Act（执行工具）→ Observe（观察结果）→ Repeat（回到 Think）。禁止一次性 if-else 规则引擎。 |
| R4 | **Workflow 真实 OpenJiuwen 集成** | P1 | ⚠️ 待实现 | 当前 Workflow 代码生成主要针对 Agent 优化。Workflow 需对接 OpenJiuwen WorkflowComponent / Runner API。 |
| R5 | **OpenJiuwen API 盘点文档** | P1 | ⚠️ 待完成 | PRD §8.2 要求的 `docs/technical/openjiuwen_api_inventory.md` 尚未编写。当前实现基于源码阅读（`agent-core/`），非正式 API 文档。 |
| R6 | **生成产物目录对齐 PRD** | P1 | ⚠️ 部分偏离 | PRD §FR-003 要求的 `config/`、`examples/` 目录当前未生成。openjiuwen 安装方式从"项目内 adapter"改为"沙箱预装"，PRD 目录结构需更新。 |
| R7 | **前端运行阶段展示 ReAct 过程** | P2 | ⚠️ 待实现 | 当前测试台只展示最终 `reply`，不展示中间的 Think（推理链）、Tool Call（工具调用）、Tool Result（工具结果）过程。前端应逐轮展示 ReAct 循环的每个步骤。 |
| R8 | **README 生成质量** | P2 | ⚠️ 质量波动 | opencode 生成的 README 质量取决于 prompt 和模型。需在 prompt 中增加 README 内容清单（§14.4）。 |

### 0.2 分支策略

| 分支 | 用途 | OpenJiuwen | 运行 LLM |
|---|---|---|---|
| `main` | 主开发线 | prompt 生成 adapter | ✅ 真实 API |
| `feature/openjiuwen-real` | 完整 openjiuwen 开发 | Docker 预装 0.1.15 | ✅ 真实 API |
| `demo/lightweight-runtime` | 轻量演示 | 自研 `openjiuwen_runtime` 包 (17 tests) | ✅ 真实 API |
...
4. 系统生成 **基于真实 OpenJiuwen 框架** 的 Python 工程代码（禁止 mock adapter、禁止纯规则引擎）。
...
当前实现采用 **Docker 沙箱预装** 模式，而非项目内 adapter 目录：

| 分支 | 适配方式 | 安装位置 |
|---|---|---|
| `feature/openjiuwen-real` | 真实 openjiuwen 0.1.15 | Docker 镜像（`pip install /tmp/agent-core`） |
| `demo/lightweight-runtime` | 自研 `openjiuwen_runtime` 包 | Docker 镜像（`pip install /tmp/openjiuwen-runtime`） |
| `main` | opencode 动态生成 `src/openjiuwen_runtime/__init__.py` | 项目内（每次生成现写） |

**真实 openjiuwen 分支的生成代码使用以下 API**：
...
### 8.4 生成代码中的 OpenJiuwen 约束（P3 更新）

生成出的 Agent/Workflow 项目必须通过 OpenJiuwen SDK 创建和运行。
...
标准目录（P3 更新 — openjiuwen 已预装在 Docker 沙箱，不再生成 `openjiuwen_runtime/` 目录）：
...
**与 PRD v0.3 原文的关键差异（P3 演进）：**

6. **运行阶段接入真实 LLM**：生成 Agent 在测试台运行时通过环境变量注入的 DEEPSEEK_API_KEY 调用大模型，不再使用 mock 响应。
7. **真实 OpenJiuwen 框架**：`feature/openjiuwen-real` 分支在 Docker 沙箱中预装完整的 openjiuwen 0.1.15，生成代码直接使用 `ReActAgent` + `@tool` + `invoke()`。
8. **ReAct 循环替代规则引擎**：生成 Agent 的核心逻辑是 Think → Act → Observe → Repeat 循环，由 LLM 驱动工具调用，不再是 if-else 关键词匹配。
9. **openjiuwen_runtime 演进**：从 `main` 分支的”每次生成现写”演进为 `demo/lightweight-runtime` 的”预编译包”，最终到 `feature/openjiuwen-real` 的”完整框架”。
10. **目录结构简化**：`src/openjiuwen_runtime/` 不再作为项目文件生成（已在 Docker 镜像中预装）。

本版本成败标准：

> 是否能清晰演示”通过自然语言生成基于 OpenJiuwen 的 Python Agent/Workflow 代码，并能运行、测试、查看源码和导出”，**且运行阶段由真实 LLM 驱动 ReAct 循环**。

---

## 18. P3 实现对照 Review（2026-07-13）

以下逐项对照 PRD 要求审查 `feature/openjiuwen-real` 分支的实际实现。

### 18.1 总评

当前实现已满足 P0 核心闭环。最关键的三项 P0 新增需求（R1/R2/R3）已实现：运行阶段真实 LLM、基于 OpenJiuwen 框架、ReAct 循环。以下详述各模块对照情况。

### 18.2 已实现（✅）

| PRD 条款 | 状态 | 实现位置 |
|---|---|---|
| §4.1 #4 — 生成 OpenJiuwen Python 工程 | ✅ | `opencode-engine.ts:buildPrompt()` |
| §4.1 #5 — ReAct 循环 | ✅ | prompt 指定 `ReActAgent` + `invoke()` + Think→Act→Observe |
| §4.1 #6 — 运行阶段真实 LLM | ✅ | `run.service.ts:buildRunEnv()` 注入 DEEPSEEK_API_KEY |
| §4.1 #9 — 效果测试台 | ✅ | `POST /api/generations/:id/agent/runs` |
| §8.4 — 禁止 mock/规则引擎 | ✅ | prompt 约束：”不得自己写 if-else 规则引擎” |
| §8.4 — 禁止 `from src.xxx` | ✅ | prompt 约束：”严禁 `from src.`” |
| §8.4 — 禁止硬编码密钥 | ✅ | prompt 指定 `os.getenv(“DEEPSEEK_API_KEY”)` |
| §10.1 — 可演示性（mock fallback） | ✅ | `OPENCODE_ALLOW_FALLBACK=true` |
| §10.3 — 安全（密钥不进源码） | ✅ | env var 注入 + `redactBuffer()` 脱敏 |
| §FR-006 — smoke test gate | ✅ | `orchestrator.service.ts:smokeTest()` |
| §FR-012 — 失败不覆盖成功版本 | ✅ | `D-002` 逻辑 |

### 18.3 部分实现或偏离（⚠️）

| PRD 条款 | 状态 | 差距 |
|---|---|---|
| §FR-003 目录结构 | ⚠️ | 不生成 `config/`、`examples/`、`.env.example`；不生成 `src/tools/`（工具定义在 `agent.py` 内） |
| §FR-004 Workflow 代码生成 | ⚠️ | Workflow prompt 简化，未完整对接 OpenJiuwen WorkflowComponent API |
| §8.2 API 盘点 | ⚠️ | `docs/technical/openjiuwen_api_inventory.md` 未编写 |
| §8.3 适配层 | ⚠️ | 架构已从”项目内 adapter 目录”变为”Docker 预装”模式，PRD 描述已更新 |
| §14.4 README 内容 | ⚠️ | opencode 生成的 README 质量依赖 prompt，无强制内容清单校验 |
| §FR-005 生成过程可观测 | ⚠️ | 左侧事件流已实现，但 opencode 内部 stdout/stderr 的行级解析不完整（v1 新增了 `→ Read/← Write` 解析，但 Traceback 类错误可能漏报） |

### 18.4 待实现（❌）

| PRD 条款 | 说明 |
|---|---|
| R4 — Workflow 真实 OpenJiuwen 集成 | `src/workflows/workflow.py` 的 prompt 需指定 `WorkflowComponent` + `Runner.run_workflow()` |
| R5 — API 盘点文档 | 见 §8.2 |
| R7 — 前端展示 ReAct 过程 | 当前 `run.service.ts` 只返回 `{“reply”: “...”}`，不返回中间 tool_calls 细节给前端渲染 |
| R8 — README 质量 | 建议在 lint gate 中增加 README 内容检查 |

### 18.5 技术债务

1. **`run.service.ts:buildRunEnv()`** — 只注入了 DEEPSEEK 系列 env var。若后续支持多 provider，需扩展。
2. **`opencode-engine.ts:buildPrompt()`** — Agent 模板代码使用字符串拼接，可维护性差。建议后续抽取为独立模板文件。
3. **Workflow 代码生成** — 当前 prompt 以 Agent 为主，Workflow 模板的 `_realWorkflowPySpec()` 仅返回占位内容。
4. **python_runner 与 openjiuwen 的协作** — `run.service.ts` 通过 `python -m python_runner.cli agent run --project .` 启动，python_runner 期望 `run_agent()` 返回 `dict`；openjiuwen 的 `ReActAgent.invoke()` 返回 `{“output”: “...”, “result_type”: “answer”}`。当前转换在生成的 `agent.py` 中由 opencode 生成的 wrapper 处理，无统一规范。
5. **Docker 镜像体积** — `feature/openjiuwen-real` 安装了完整 openjiuwen（含 sqlalchemy, pymilvus, transformers 等），镜像约 2GB+。后续可考虑 openjiuwen 的 minimal install option。
