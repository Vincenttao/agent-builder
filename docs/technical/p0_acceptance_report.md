...
## 8. P1 下一阶段能力状态（2026-07-09）

依据 `docs/technical/p1_llm_opencode_execution_plan.md`，P1 端到端竖切已落地，分支 `feat/p1-llm-opencode`。自动化测试全绿：API 128、web 20、python 10、Playwright E2E 4（含 2 条非示例链路）。

| 阶段 | 能力 | 状态 |
| --- | --- | --- |
| Phase 9 | 异步 LLM Spec Parser（deterministic/llm/hybrid 三模式）+ `generation_specs` 持久化 + `POST /api/generations` 立即返回 + 非示例 prompt 不再 `PROMPT_PARSE_FAILED` | ✅ |
| Phase 9 | `MockLlmSpecParser`（CI/E2E）+ `OpenAiCompatibleSpecParser`（真实 OpenAI 兼容网关，带重试/超时/JSON-fence 提取）+ `SPEC_LLM_*` env | ✅ |
| slice H | 导出过滤补齐：`.agent_builder/`、`.opencode/`、`opencode.json`、`*opencode*.json` 全部排除 | ✅ |
| Phase 11 | 非示例 Agent / Workflow 后端 integration（mock LLM → Spec → pipeline completed）+ 持久化 Spec 不二次解析 | ✅ |
| Phase 12 | 通用 mock runtime（`MockAgent` 按 `spec.name/system_prompt/tools` 生成回复，不再硬编码塔罗/售前）+ 通用 smoke test | ✅ |
| Phase 12 | 非示例 Playwright E2E（天气 Agent + 合同审核 Workflow：generate→source→run→export，run 不泄漏 Demo 文案） | ✅ |
| Phase 12 | 完成摘要展示 parser mode / codegen engine / fallback 状态 | ✅ |
| Phase 10 | `CODEGEN_ENGINE` env 接入 orchestrator + 生成物 contract lint 门禁（必需文件 / 禁止 langgraph·crewai·dify / secret） | ✅ |
| Phase 10 | 真实 `opencode run` 经 SandboxService 执行（含网络策略/secret 脱敏/超时） | ⏳ 结构已留接口（mock-mode 事件 + TemplateEngine fallback 已可用）；真实 opencode run 需 opencode 自身模型配置，作为手动验收路径 |
| Phase 13 | `.env.example`（全量 `SPEC_LLM_*`/`OPENCODE_*`/`FEATURE_*`）+ README | ✅ |
| Phase 14-15 | 版本/Diff/修复重试/日志查看/Spec 确认/模板库/历史/模型配置页 | ⏳ 按计划属第 3/4 个 PR，未在本竖切实现 |

**真实 LLM 验收**：`.env` 中设置 `SPEC_LLM_PROVIDER=openai-compatible` + `SPEC_LLM_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1` + `SPEC_LLM_API_KEY` + `SPEC_LLM_MODEL`（需使用该 key 有权限的模型），即可让非示例 prompt 走真实 LLM 解析。CI/E2E 默认 `SPEC_LLM_PROVIDER=mock`，无密钥即可跑通。
