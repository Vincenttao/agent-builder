...
export class OpenCodeEngine implements CodeGenerationEngine {
  ...
  private buildPrompt(spec: AgentSpec | WorkflowSpec): string {
    ...
    const lines = [
      ...
      '## 沙箱环境',
      '',
      '## 沙箱环境（所有依赖已预装，禁止 pip install）',
      '',
      '沙箱 **无外网访问**，以下包已预装，不得执行任何 pip install 命令：',
      '- Python 3.11 + pytest, setuptools, wheel, openai, requests, httpx, python-dotenv, pydantic, pyyaml, jinja2, loguru',
      '- **openjiuwen 0.1.15**（完整框架）：`ReActAgent`, `ReActAgentConfig`, `AgentCard`, `@tool`, `Runner`',
      '- **python_runner**：平台的 runner CLI，不要自己实现',
      '',
      '🔴 **严禁 pip install**：所有 `pip install xxx` 都会因网络限制而失败，浪费时间。',
      '如果代码需要某个包，只能使用上面列出的已预装包。',
      '生成的 pyproject.toml 依赖列表也只能包含这些包。',
      '',
      ...
      '',
      '1. **严禁 pip install openjiuwen**：openjiuwen 已预装，',
      '   不要执行 `pip install openjiuwen` 或添加 openjiuwen 到 pyproject.toml 依赖。',
