...
export class OpenCodeEngine implements CodeGenerationEngine {
  ...
  private buildPrompt(spec: AgentSpec | WorkflowSpec): string {
    ...
    const lines = [
      ...
      '',
      '────────────────────────────────────────────────────',
      '## src/openjiuwen_runtime/__init__.py — ReAct Agent 适配层',
      '────────────────────────────────────────────────────',
      '',
      '这个文件是核心！它必须实现一个真正的 ReAct Agent 循环（不是 mock/stub）。',
      '用 openai 库调用大模型，实现 Think → Act → Observe → Repeat 循环。',
      '',
      '必须提供的类和函数：',
      '',
      '### ToolCard',
      ...
      '    """装饰器：把函数变成 ToolCard。parameters 必须是 JSON Schema dict。',
      '    示例: parameters={"type":"object","properties":{"text":{"type":"string"}},"required":["text"]}',
      '    """',
      '    def decorator(func):',
      '        return ToolCard(name=name, description=description, parameters=parameters, func=func)',
      '    return decorator',
      '```',
      '',
      '### ReActAgent — 核心循环',
      '```python',
      'class ReActAgent:',
      '    def __init__(self, system_prompt: str, tools: list[ToolCard], max_iterations: int = 5):',
      '        # 从环境变量读取 LLM 配置',
      ...
      '        """执行 ReAct 循环，返回 {"reply": str, "tool_calls": list}"""',
      '        messages = [',
      ...
      '            if msg.tool_calls:',
      '                # Act: 执行 LLM 请求的工具调用',
      '                messages.append({"role": "assistant", "tool_calls": [...]})  # 序列化',
      ...
      '                # Observe 后回到循环顶部，LLM 看到工具结果后继续思考',
      '            else:',
      '                # Think 完成：LLM 认为不需要更多工具调用，生成最终回复',
      '                return {"reply": msg.content or "", "tool_calls": tool_calls_made}',
      '',
      '        return {"reply": "已达到最大迭代次数", "tool_calls": tool_calls_made}',
      '',
      ...
      '',
      '关键要求：',
      '- assistant 消息中的 tool_calls 必须正确序列化为 OpenAI 格式',
      '- tool 执行结果作为 tool role 消息添加，tool_call_id 必须匹配',
      '- 循环在 LLM 不再返回 tool_calls 时正常退出',
      '- 达到 max_iterations 时兜底返回',
      '',
      '────────────────────────────────────────────────────',
