...
export class OpenCodeEngine implements CodeGenerationEngine {
  ...
  private buildPrompt(spec: AgentSpec | WorkflowSpec): string {
    ...
    const lines = [
      ...
      '',
      '如果确实需要上述列表之外的包，可以在生成阶段 pip install（有网络）。',
      '但注意：生成的 Agent 在运行阶段**无网络**，所以运行期依赖必须全部在 pyproject.toml 中声明',
      '且必须在镜像中预装。如果缺少，Agent 运行时会报 ModuleNotFoundError。',
      '最好的策略：优先使用已预装包的组合完成业务逻辑。',
      '',
